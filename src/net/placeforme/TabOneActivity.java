...
public class TabOneActivity extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater,
		...
		View view = inflater.inflate(R.layout.fragment_tab_one, container,
				false);

		final EditText tituloText = (EditText) view
				.findViewById(R.id.busca_titulo);
		final EditText dataText = (EditText) view.findViewById(R.id.busca_data);
		final Spinner grupoCampoSpinner = (Spinner) view
				.findViewById(R.id.busca_grupo);
		...
		ArrayAdapter<String> arrayAdapterGrupo = new ArrayAdapter<String>(
				container.getContext(), android.R.layout.simple_spinner_item,
				arrayListStringGrupo);
		...
		buttonFiltro.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				int grupo_id = arrayListGrupo.get(
						grupoCampoSpinner.getSelectedItemPosition())
						.getGrupoId();

				TabTwoActivity.populaListaStaticFiltros(titulo, data, grupo_id);
				TabTreeActivity.populaListaStaticFiltros(titulo, data, grupo_id);

				MainActivity.actionBarStatic
						.selectTab(MainActivity.actionBarStatic.getTabAt(1));
