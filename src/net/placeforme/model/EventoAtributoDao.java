...
public class EventoAtributoDao {
	...
	private String[] ALLCOLUMNS = new String[] {
			DbHelper.TABLE_EVENTOATRIBUTO_ID,
			DbHelper.TABLE_EVENTOATRIBUTO_TEXTO,
			DbHelper.TABLE_EVENTOATRIBUTO_EVENTO_ID,
	...
	public void add(EventoAtributo eventoAtributo) {
		...
		values.put(DbHelper.TABLE_EVENTOATRIBUTO_TEXTO,
				eventoAtributo.getTexto());
		values.put(DbHelper.TABLE_EVENTOATRIBUTO_EVENTO_ID,
				eventoAtributo.getEventoId());
		values.put(DbHelper.TABLE_EVENTOATRIBUTO_ATRIBUTO_ID,
				eventoAtributo.getAtributoId());
	...
	public EventoAtributo get(int id) {
		...
		Cursor cursor = database.query(DbHelper.TABLE_EVENTOATRIBUTO,
				ALLCOLUMNS, DbHelper.TABLE_EVENTOATRIBUTO_ID + "=?",
	...
	public int update(EventoAtributo eventoAtributo) {
		...
		values.put(DbHelper.TABLE_EVENTOATRIBUTO_ATRIBUTO_ID,
				eventoAtributo.getAtributoId());
		int ret = database.update(DbHelper.TABLE_EVENTOATRIBUTO, values,
				DbHelper.TABLE_EVENTOATRIBUTO_ID + " = ?",
				new String[] { String.valueOf(eventoAtributo
						.getEventoAtributoId()) });
	...
	public void delete(EventoAtributo eventoAtributo) {
		...
		database = dbHelper.getWritableDatabase();
		database.delete(DbHelper.TABLE_EVENTOATRIBUTO,
				DbHelper.TABLE_EVENTOATRIBUTO_ID + " = ?",
				new String[] { String.valueOf(eventoAtributo
						.getEventoAtributoId()) });
		dbHelper.close();
	...
	public int count() {
		String countQuery = "SELECT " + DbHelper.TABLE_EVENTOATRIBUTO_ID
				+ " FROM " + DbHelper.TABLE_EVENTOATRIBUTO;
		database = dbHelper.getReadableDatabase();
	...
	public ArrayList<EventoAtributo> getAllByEvento(Evento evento) {
		...
		database = dbHelper.getReadableDatabase();
		Cursor cursor = database.query(DbHelper.TABLE_EVENTOATRIBUTO,
				ALLCOLUMNS, DbHelper.TABLE_EVENTOATRIBUTO_EVENTO_ID + "=?",
