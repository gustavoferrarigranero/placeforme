...
public class FotoDao {
	...
	public void add(Foto foto) {
		...
		values.put(DbHelper.TABLE_FOTO_FOTO,
				Utils.BitMapToString(foto.getFoto()));
	...
	public Foto get(int id) {
		...
		Cursor cursor = database.query(DbHelper.TABLE_FOTO, ALLCOLUMNS,
				DbHelper.TABLE_FOTO_ID + "=?",
	...
	public List<Foto> getAll() {
		...
		database = dbHelper.getReadableDatabase();
		Cursor cursor = database.query(DbHelper.TABLE_FOTO, ALLCOLUMNS, null,
				null, null, null, null);
	...
	public int update(Foto foto) {
		...
		values.put(DbHelper.TABLE_FOTO_EVENTO_ID, foto.getEventoId());
		values.put(DbHelper.TABLE_FOTO_STATUS, foto.getStatus());
		int ret = database.update(DbHelper.TABLE_FOTO, values,
				DbHelper.TABLE_FOTO_ID + " = ?",
	...
	public int count() {
		String countQuery = "SELECT " + DbHelper.TABLE_FOTO_ID + " FROM "
				+ DbHelper.TABLE_FOTO;
		database = dbHelper.getReadableDatabase();
